...
const usePosts = (initialPage = 1): UsePosts => {
    const [posts, setPosts] = useState<Post[] | null>(null);
    const [isFetching, setIsFetching] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [pagination, setPagination] = useState<PostResponse["pagination"] | null>(null);

    const getPosts = async (page = 1, limit = 8): Promise<void> => {
